...
sleep 5
enclave_cid=$(grep "N successfully connected from local address AF=40 cid:" /tmp/socat.log | awk -F'cid:' '{print $2}' | awk '{print $1}')
if [ -n "$enclave_cid" ]; then
    echo "Enclave CID: $enclave_cid" | tee /home/user/.arbitrum/enclave_cid.log
else
    echo "No client connection found in logs"
fi
