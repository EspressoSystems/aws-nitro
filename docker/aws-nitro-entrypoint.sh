...
socat -d -d TCP-LISTEN:2049,bind=127.0.0.1,fork,reuseaddr,so-keepalive,keepidle=60,keepintvl=30,keepcnt=5,rcvbuf=15M,sndbuf=15M VSOCK-CONNECT:3:8004,so-keepalive,connect-timeout=10,retry=5,interval=10,rcvbuf=15M,sndbuf=15M &> /tmp/socat.log &
sleep 1
